...
*Cập nhật 08/09/2026 (lần 8). Số liệu **đo từ mã**, không gõ tay: `eide spec`,
...
**128/238 năng lực (54%). Mốc M0 đóng 22/22; M1 đạt 74/75 (99%), còn 1. 957 test xanh trên
...
| M2 | 32/99 · 32% | Sinh mã, board, mô phỏng nền, tài liệu đầy đủ |
...
`env` 6/7 · `code` 10/16 · `passport` 4/8 · `diagram` 4/14 · `doc` 3/12 · `archive` 4/4 · `registry` 1/5 ·
`report` 1/4
...
| **Z-05** thêm tính năng | 8/16 | `code.modify` (M2) — `generate_module`…`merge` đã xong |
...
**Ba lớp test, ba loại câu hỏi khác nhau.** `make check` (957 test, giả lập) hỏi *"mã có đúng
