...
*Cập nhật 08/09/2026 (lần 9). Số liệu **đo từ mã**, không gõ tay: `eide spec`,
...
**131/238 năng lực (55%). Mốc M0 đóng 22/22; M1 đạt 74/75 (99%), còn 1. 969 test xanh trên
...
| M2 | 35/99 · 35% | Sinh mã, board, mô phỏng nền, tài liệu đầy đủ |
...
`env` 6/7 · `code` 13/16 · `passport` 4/8 · `diagram` 4/14 · `doc` 3/12 · `archive` 4/4 · `registry` 1/5 ·
`report` 1/4
...
| Chỉ số | Giá trị |
|---|---|
| Test Python | **969** xanh, arm64 + x86_64 (`make check`) |
...
**Ba lớp test, ba loại câu hỏi khác nhau.** `make check` (969 test, giả lập) hỏi *"mã có đúng
