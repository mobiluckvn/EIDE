...
c.push(SP());
c.push(P('**Hai loại cổng, và khác biệt nằm ở HIỆN VẬT chứ không ở mức nguy hiểm.** Mọi cổng đều xét một hiện vật: G-OPS xét `board`/`artifact`, G-SRC xét `source`, G3 xét `patch`, G5 xét `pkg`, G1 xét `plan`, G-FACT xét `fact`. Câu hỏi phân loại là hiện vật ấy có TỒN TẠI TRƯỚC lời gọi hay không. (a) `target.flash` nhận `artifact` làm tham số, nên Router đánh giá G-OPS được ngay ở cửa — đúng mô hình mà đoạn trên mô tả, "đặc trưng do năng lực cung cấp khi gọi Router". (b) `plan.create` SINH RA bản kế hoạch mà G1 xét (`plan.steps`, `plan.all_cited`, `plan.arch_change`), và `kg.review_facts` xét từng fact nó đang duyệt. Hiện vật chưa tồn tại lúc Router quyết định, nên hỏi cổng ở cửa chỉ cho ra một quy tắc bắt hết — luôn ASK. Tệ hơn: `kg.review_facts` bị chính cổng nó phục vụ chặn lại nên không fact nào được duyệt bao giờ, và `plan.create` không bao giờ chạy nên không bao giờ có kế hoạch để G1 xét. Vì vậy cổng loại (b) được năng lực gọi **bên trong**, đúng như hợp đồng của chúng viết (CDS-12.1 PLAN-03 "policy.decide(G1) với đặc trưng steps/all_cited/…"; CDS-12.2 KG-05 "với mỗi fact chạy policy.decide(G-FACT)"). Cùng vòng luẩn quẩn với `chat.clarify` (DEV-020) và `tool.write` (DEV-026), lần này ở tầng hiện vật. Xem DEVIATIONS DEV-054.'));
...
// POL-17 §3 — linh kiện làm board CHUYỂN ĐỘNG hoặc SINH NHIỆT khi mã chạy sai. `board.mark_lab`
// đối chiếu netlist với bảng này trước khi tin lời khai `no_actuator` của người.
// [khóa, tên nhóm, mẫu mã (khớp không phân biệt hoa thường, ở bất kỳ đâu trong mpn/value/footprint)]
const ACTUATORS = [
  ['motor_driver', 'Mạch lái động cơ', ['DRV8\\d{3}', 'L29[38]', 'TB6612', 'A4988', 'BTS7960', 'VNH\\d', 'MP6\\d{3}']],
  ['actuator', 'Cơ cấu, rơ-le, gia nhiệt', ['ULN2\\d{3}', 'RELAY', 'SERVO', 'STEPPER', 'SOLENOID', 'MOTOR', 'PUMP', 'VALVE', 'HEATER']],
  ['power_switch', 'MOSFET công suất', ['IRF\\d{3,4}', 'IRLZ\\d+', 'AO3400', 'SI2302']],
];
...
c.push(SP());
c.push(SP());
c.push(P('**Nhận dạng cơ cấu chấp hành.** Lời khai của người ở `board.mark_lab` có thể sai — người khai không nhất thiết là người vẽ mạch — nên nó được đối chiếu với netlist. Tệp `policy/actuators.yaml` kèm theo liệt kê các mẫu mã linh kiện làm board **chuyển động hoặc sinh nhiệt** khi mã chạy sai: mạch lái động cơ, rơ-le và cơ cấu, MOSFET công suất. Bảng ấy là lưới chặn **một chiều**: bắt được thì chắc chắn không phải board lab, còn không bắt được thì không kết luận gì và lời khai của người vẫn là điều kiện bắt buộc. Đặt ngược lại — tự cho lab khi không thấy động cơ — thì một MOSFET lái van sẽ lọt, và cái lọt ấy chuyển động. Xem DEVIATIONS DEV-068.'));
c.push(T([2300, 7000], ['Nhóm', 'Mẫu mã linh kiện'], ACTUATORS.map(g => [g[1], g[2].join(', ')])));
c.push(SP());
