...
c.push(SP());
c.push(P('**Từ v1.3: GEditor là thành phần TRONG kho EIDE** (`apps/geditor/`), không còn là phụ thuộc ngoài. Chủ sản phẩm quyết định 06/09/2026 fork thay vì tham chiếu, vì hai kho đi hai hướng phát triển khác nhau. Hệ quả cho tài liệu này: các mục [GE?] không còn là "yêu cầu gửi sang một sản phẩm khác" mà là việc trong cùng kho; phần bàn về khung plugin bên thứ ba chỉ còn giá trị lịch sử. Về bí mật: khóa riêng ký cập nhật Sparkle **không** được mang sang — nó là quyền ký một bản cập nhật tự động mà mọi bản đã cài sẽ tự nuốt, nên nhân thêm một nơi lưu là nhân thêm một nơi mất (SEC-25). `.gitignore` chặn `secrets/*_private*`. Kho Geditor gốc giữ nguyên khóa cũ theo quyết định 06/09/2026, với điều kiện kho còn private — đổi sang public thì phải đổi khóa trước. Xem DEVIATIONS DEV-004.'));
