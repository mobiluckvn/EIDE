...
entity("Preference", "preference", "Tùy chọn người dùng (D8). Bảng này giữ RIÊNG `scope=project`; `scope=user` nằm ở `preferences.yaml` trong thư mục cấu hình người dùng (§6). Phân công theo phạm vi chứ không tùy chọn: bảng nằm trong store của TỪNG dự án, nên một tùy chọn `user` ghi ở dự án A sẽ không đọc được ở dự án B — trái đúng nghĩa của \"user\". Xem DEVIATIONS DEV-010.", [("key", S, "TEXT NOT NULL", 1, "", None), ("scope", S, "TEXT NOT NULL", 1, "chỉ `project` trong bảng này", ["user","project"]), ("value", O, "TEXT NOT NULL", 1, "JSON — giá trị bất kỳ (số, chuỗi, mảng, đối tượng), KHÔNG chỉ đối tượng; `learned_from`/`ttl_days` là trường ngang hàng, không lồng bên trong (DEV-009)", None), ("learned_from", S, "TEXT", 0, "decision/turn id", None), ("ttl_days", I, "INTEGER", 0, "", None), ("at", DT, "TEXT NOT NULL", 1, "", None)], pk="(key, scope)", since="M1")
...
YAML_FILES = [
 ...
 ("preferences.yaml", "Tùy chọn phạm vi `user` (M6) — phạm vi `project` ở bảng `preference` (§2.x)", "map key → {value, learned_from, ttl_days, at}", "M1"),
