...
**Tôi (người dùng):** bấm Hoàn tác cho `1147dcf08c2d`

**Tác tử trả lời** *(sau 2.7 s)*:

```
VÙNG TRAO ĐỔI  ▁ ▂ ▃ Sẵn sàng. Gõ một câu tiếng Việt; tôi nói lại ý hiểu trước khi làm.  Đã mở `do-nhiet-do-sen42-qua-i2c` — 0 tính năng trong hồ sơ. Gõ một câu tiếng Việt để bắt đầu.  Không có màn nào mang tiền tố `LichSu` trong danh mục 25 màn.  Không hoàn tác được: E2000: Không đọc được `undo_ref` `1147dcf08c2d` — cần `commit:<sha>` hoặc `run:<id>`   Thử: đối chiếu FR-SNS-01 với phần cứng thật Gửi 
```
