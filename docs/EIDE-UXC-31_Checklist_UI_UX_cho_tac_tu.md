...
| Phiên bản | **1.3 — 21/09/2026** (v1.0 17/09; v1.3 sửa năm chỗ tài liệu sai theo [DEV-138], [DEV-139], [DEV-142] ×2, [DEV-143] — mỗi chỗ ghi rõ v1.0 đã viết gì) |
...
- [x] **2D.6** Trước mọi chuỗi: tác tử in "Ý hiểu (chat.restate)" + danh sách bước dự kiến + hai nút "Đúng — làm đi" / "Sửa ý hiểu" (A1); mức A2–A3 tự chạy nhưng vẫn in ý hiểu.  ·  **MỘT NỬA, và nửa kia bị chặn ở tầng dưới — [DEV-140]. Thẻ Ý hiểu in câu `chat.restate` + bước đánh số (daemon nay trả `{restate, steps}` kèm `chat.send`). Hai nút thì KHÔNG hiện: `chat.orchestrate` dựng chuỗi và chạy nó trong cùng một lời gọi, nên không có chỗ dừng để gật đầu. Thẻ nói thẳng điều đó thay vì hiện nút bấm vào không đảo được gì**
...
- [x] **2E.6** Run kết thúc: `run.done` → tác tử in báo cáo (chat.report_back) ngay dưới thẻ: sản phẩm, cổng đã qua, chi phí, các mục hoàn tác được. `run.cancelled` → thẻ ghi ai huỷ, lúc nào.  ·  **✔ `run.done` → `chat.report_back` in ngay dưới thẻ; `run.cancelled` ghi AI huỷ và LÚC NÀO. Cả hai chỉ một lần cho mỗi lượt chạy — `run.done` tới nhiều lần khi nghe lại sổ cái**
