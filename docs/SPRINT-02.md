...
| EXTRACT-05 | `extract.dt_binding` | 12.2 | **Xong 10/09** (M2, khối D) — 8 test, arm64 + x86_64. Parser thuần, tầng bạc (K4 của KAD-07 §5.1). Đọc cả hai dạng binding đang tồn tại: Zephyr (`required: true` trong từng thuộc tính) và dt-schema Linux (`required:` là danh sách cấp cao). `compatible: "bosch,bme280"` → IRI `chip:bosch.bme280` theo khuôn `<ns.part>` của KAD-07 §6.1 |
| EXTRACT-17 | `extract.bom` | 12.2 | **Xong 10/09** (M2, khối D) — 10 test, arm64 + x86_64. Bốn đường vào dùng lại thứ đã có: netlist (`doc_netlist` của EXTRACT-16), csv, xlsx (`_xlsx` của EXTRACT-19), readme (mô hình). Gộp theo **MPN đầy đủ** vì BOM là danh sách mua — [DEV-078](DEVIATIONS.md); đường ảnh chờ D3 — [DEV-079](DEVIATIONS.md) |
| EXTRACT-18 | `extract.bom_enrich` | 12.2 | **Xong 10/09** (M2, khối D) — 7 test, arm64 + x86_64. Hộ chiếu trong store thắng mọi ứng viên tải về (nó đã qua G-FACT); tra bằng `mpn_base`; `search.registry` là M4 nên chuỗi giảm cấp hiện bắt đầu từ `search.vendor`, thứ tự giữ nguyên để chèn được sau. Không tra ra → `kg.request`, một MPN một yêu cầu |
| EXTRACT-11 | `extract.pdf_formula` | 12.2 | **Xong 10/09** (M2, khối D) — 7 test, arm64 + x86_64. Công thức bù nhiệt → **K5 skill** (`.eide/skills/<part>-<mục>.md`, front-matter `applies_to`, mục Nguồn có số trang), **không thành fact**: một thủ tục không có `subject` để tra. Chỉ đưa mô hình đúng mục được nêu. E5002 khi không có mã C → [DEV-080](DEVIATIONS.md) |
...
| **WI-257** | **Người:** ký danh sách trắng nguồn/gói/board lab | Ký lần đầu 06/09 (DEV-001). **11/09 đợt 2 cần ký LẠI**: `defaults.yaml` thêm `raw.githubusercontent.com` — nơi SVD tầng vàng thật sự nằm, mà G-SRC-01 khớp CHÍNH XÁC nên `github.com/cmsis-svd` không phủ được. Mã + test + [DEV-087] đã xong; chưa ký thì PolicyGate bỏ hẳn ba danh sách, 19 test đỏ cùng một nguyên nhân. `.venv-arm/bin/python -m eide.cli policy sign --by "Vũ Trí Công"` |
