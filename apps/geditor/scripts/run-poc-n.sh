#!/bin/bash
# PoC-N — PDFKit cho khung xem PDF có phá ADR-08 (khởi động ≤ 500 ms) không?
#
#   scripts/run-poc-n.sh
#
...
# PDFKit là framework nặng theo đúng nghĩa ấy: nó có engine dựng hình PDF riêng bên trong.
#
...
ARCH="$(uname -m)"
RESULT="benchmarks/results/poc-n-${ARCH}.json"
RUNS=15
...
echo "▸ PoC-N — giá liên kết PDFKit lúc nạp"
echo
...
int main(int argc, char **argv) {
    ...
    for (uint32_t i = 0; i < images; i++) {
        ...
        if (n && strstr(n, "PDFKit")) { loaded = 1; break; }
    }
    ...
    if (argc > 1 && argv[1][0] == 'l') {
        ...
        void *h = dlopen("/System/Library/Frameworks/PDFKit.framework/PDFKit", RTLD_LAZY);
        uint64_t t1 = mach_absolute_time();
        lazy = h ? (double)(t1 - t0) * tb.numer / tb.denom / 1e6 : -1;
    }
    printf("{\"preMainMs\": %.3f, \"images\": %u, \"pdfkitLoaded\": %d, \"lazyDlopenMs\": %.3f}\n",
...
clang -O2 -framework Foundation -framework AppKit -framework PDFKit \
...
printf "   -framework PDFKit %7s ms   (dải %s … %s)\n" "$LINKED_MED" "$LINKED_LO" "$LINKED_HI"
...
d, n = $DELTA, $NOISE
if abs(d) <= n:
    print('   Chênh lệch %.3f ms KHÔNG vượt sàn nhiễu %.3f ms — liên kết PDFKit không đo được' % (d, n))
    ...
    print('   ⚠ Nhưng KHÔNG kết luận \"PDFKit miễn phí\": giá thật nằm ở lần dựng PDFView đầu')
