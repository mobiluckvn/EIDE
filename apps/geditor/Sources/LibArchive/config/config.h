/*
 * config.h cho libarchive 3.7.7 trong GEditor.
 *
 * GEDITOR_VENDOR_VERSION "3.7.7"   ← scripts/vendor-libarchive.sh kiểm dòng này
 *
 * ============================================================================
 * TỆP NÀY DO CMAKE CỦA UPSTREAM SINH RA MỘT LẦN, RỒI SỬA ĐÚNG HAI DÒNG
 * ============================================================================
 *
 * Khác với Sources/LZMA/config/config.h (viết tay, 42 macro): ở đây libarchive đọc tới hơn
 * bốn trăm định nghĩa, phần lớn là "libc của hệ này có hàm X không". Viết tay ngần ấy là tự
 * chuốc một lớp sai âm thầm — một `HAVE_` đặt nhầm không làm đứt bản dựng, nó chỉ lặng lẽ đổi
 * nhánh mã được biên dịch.
 *
 * Hai thư viện chịu hai cách khác nhau vì rủi ro của chúng khác nhau: liblzma có tiền sử cửa
 * hậu chèn qua hệ build (CVE-2024-3094), libarchive thì không. Với liblzma, cái giá của việc
 * không chạy hệ build upstream là 42 dòng viết tay — trả được. Với libarchive thì không.
 *
 * LỆNH ĐÃ SINH RA TỆP NÀY (chạy lại y hệt khi nâng phiên bản):
 *
 *   cmake <nguồn libarchive> -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_BUILD_TYPE=Release \
 *     -DCMAKE_IGNORE_PREFIX_PATH="/opt/homebrew;/opt/local;/usr/local" \
 *     -DCMAKE_IGNORE_PATH="/opt/homebrew/include;/opt/homebrew/lib;/opt/local/include;\
 *                          /opt/local/lib;/usr/local/include;/usr/local/lib" \
 *     -DENABLE_OPENSSL=OFF -DENABLE_LIBXML2=OFF -DENABLE_EXPAT=OFF -DENABLE_LIBB2=OFF \
 *     -DENABLE_LZ4=OFF -DENABLE_ZSTD=OFF -DENABLE_LZO=OFF -DENABLE_MBEDTLS=OFF \
 *     -DENABLE_NETTLE=OFF -DENABLE_ACL=OFF -DENABLE_XATTR=OFF -DENABLE_PCREPOSIX=OFF \
 *     -DENABLE_TAR=OFF -DENABLE_CPIO=OFF -DENABLE_CAT=OFF -DENABLE_UNZIP=OFF \
 *     -DENABLE_TEST=OFF \
 *     -DENABLE_ICONV=ON -DENABLE_ZLIB=ON -DENABLE_BZip2=ON -DENABLE_LZMA=ON
 *
 * HAI THAM SỐ IGNORE LÀ BẮT BUỘC, KHÔNG PHẢI CHO ĐẸP. Lượt sinh đầu tiên không có chúng đã
 * bật `HAVE_BLAKE2_H` vì cmake tìm thấy libb2 của Homebrew trên máy này, và đọc `zlib`,
 * `bz2`, `lzma` từ MacPorts ở /opt/local. Kết quả là một config.h mô tả MÁY CỦA NGƯỜI CHẠY
 * chứ không mô tả máy người dùng — mà nó thì không đứt bản dựng ở đây, nó đứt ở máy khác.
 * Bằng chứng: rar5 đứt ngay lượt biên dịch đầu vì `#include <blake2.h>` không có ở nơi khác.
 *
 * `ENABLE_ICONV=ON` CŨNG LÀ BẮT BUỘC, và nó là bài học của lượt thứ hai. Lượt trước đặt OFF
 * cho "gọn", và mọi bài kiểm đều xanh — trừ một bài: tên tệp tiếng Việt trong kho .7z đọc ra
 * chuỗi RỖNG. Không có iconv thì `archive_entry_pathname_utf8()` không chuyển nổi tên về
 * UTF-8 và trả NULL, còn `archive_entry_pathname()` không chuyển nổi về charset của locale
 * (dưới `swift test` locale là "C") nên cũng trả NULL. `iconv.h` nằm sẵn trong SDK của macOS,
 * tức API công khai — không có lý do gì để tắt.
 *
 * HAI DÒNG SỬA TAY SAU KHI SINH:
 *
 *   HAVE_LZMA_H   → 1
 *   HAVE_LIBLZMA  → 1
 *
 * cmake đặt cả hai là chưa-định-nghĩa vì hai tham số IGNORE ở trên đã chặn nó thấy liblzma
 * của MacPorts — đúng như mong muốn. Nhưng GEditor CÓ liblzma: từ nguồn vendor ở
 * Sources/LZMA. Không bật hai dòng này thì bộ đọc 7z bỏ nguyên nhánh LZMA, và gần như mọi
 * tệp .7z thật đều nén bằng LZMA2.
 *
 * `zlib`, `bzip2` và `iconv` thì cmake tự tìm thấy trong SDK và bật đúng — chúng là API công
 * khai của macOS, Package.swift chỉ cần `-lz -lbz2 -liconv`.
 *
 * `HAVE_LOCALE_CHARSET` được bật nhưng KHÔNG cần link thêm `-lcharset`: chỗ duy nhất dùng nó
 * trong libarchive là archive_string.c, và điều kiện ở đó là `#if HAVE_LOCALE_CHARSET &&
 * !defined(__APPLE__)` — tức trên macOS nhánh ấy không bao giờ được biên dịch.
 *
 * Từ dòng dưới đây trở đi là nguyên văn cmake sinh, chỉ khác hai dòng đã nói.
 */
