...
----------------------------------------------------------------
01_SRS_GEditor_macOS_v2.2.docx
...
02_SAD_GEditor_macOS_v2.3.docx
...
03_STP_GEditor_macOS_v2.3.docx
...
04_UIUX_GEditor_macOS_v2.3.html
...
05_RTM_RaSoat_DongBo_GEditor_v2.3.xlsx
...
- Data Mining (FR-MIN, Phase 3-4): SRS 3.2.13 - SAD MiningEngine/
  MiningAssistant/block ```mining - STP TC-MIN-01...08 - UI/UX 6.5
  "Ban khai pha". Gom: bat thuong (z/IQR/MAD + Mahalanobis da bien,
  giai thich tung dong), phan cum k-means++/DBSCAN (goi y elbow/
  k-distance ve tai cho), luat ket hop Apriori, chuoi thoi gian
  Holt-Winters (bat buoc so baseline, thua thi noi thang), tuong quan
  + heatmap, text mining TF-IDF tokenizer tieng Viet.
  QUYET DINH KIEN TRUC: thuat toan co dien TU CAI DAT trong loi
  C++/Swift - KHONG Python/sklearn/GPU/model file. Tat dinh seed co
  dinh (ket qua tai lap bit-by-bit); chi-doc tuyet doi (checksum);
  moi ket qua kem khoi "Phuong phap". Fixture kiem thu co ground
  truth; doi chung doc lap numpy/mlxtend chay OFFLINE trong CI
  (cong cu test, khong phai dependency cua app).
- Knowledge Pack MO RONG v2.0 (FR-KNW-913...921, Phase 4): Graph sau +
  RAG Lab. (a) TRUY VAN Graph thuc thi duoc: openCypher tap con dich
  sang SQL DuckDB - SQL sinh LUON hien thi (vua dung vua hoc); thuat
  toan do thi tu cai dat (k-hop, shortest path, components, PageRank,
  Louvain) tren CSR, chuan Mining (tat dinh + huy + Phuong phap).
  (b) SOAN THAO Graph: sua truc quan tren preview ghi nguoc van ban
  (buffer la nguon su that), refactoring rename/merge/split/extract
  qua diff, schema YAML validation. (c) RAG LAB: BM25 tu cai dat
  (khong can embedding - baseline chuan cong nghiep), danh gia golden
  set recall@k/MRR/nDCG, lien ket Chunk-Entity-Graph + xuat context
  package JSONL (nen GraphRAG), nhap diem embedding ngoai de so sanh.
  Ranh gioi ADR-11 GIU NGUYEN: khong embedding/vector DB/LLM trong app.
- Python Pack (FR-PY, Phase 6, plugin thu ba): SRS Phu luc F - SAD
  Chuong 11 + ADR-13 - STP TC-PY-01...08 - UI/UX muc 16. Chay code
  Python cua NGUOI DUNG: console REPL, cmd-R chay script, buoc python
  trong Recipe. Kien truc TUYET DOI out-of-process (khong CPython
  trong tien trinh editor - crash khong lan, zero-cost khi tat).
  Cau API 'geditor': lines() streaming file GB (RSS<300MB), sql() ra
  DataFrame, duong ghi DUY NHAT la applyEdits (1 undo + diff).
  Runtime kep: dung Python co san HOAC runtime dong goi tai theo
  yeu cau (installer khong phinh). Trust theo hash cho script ngoai;
  agent-script bat buoc 2 cong (duyet code + duyet diff).
  NFR-PY-03 trung thuc: KHONG tuyen bo sandbox - noi ro voi nguoi
  dung. NFR-MIN-04 giu nguyen: loi khong phu thuoc Python.
- Mermaid Studio (FR-MMD, Phase 3-4): SRS 3.2.14 - SAD MermaidRenderer/
  Sync/VisualEdit/Toolkit - STP TC-MMD-01...06 - UI/UX muc 17.
  Soan thao so do HAI CHE DO: text (grammar, autocomplete, format giu
  comment) + TRUC QUAN (thao tac tren hinh sinh code qua applyEdits,
  1 undo) cho MOI loai so do: flowchart, sequence, class, state, ER,
  gantt, pie, mindmap, timeline, gitGraph. VAN BAN LA NGUON SU THAT -
  file .mmd van diff/Git duoc. Render mermaid.js DONG GOI OFFLINE
  (phien ban pin, khong CDN), WKWebView sandboxed khong network,
  khoi tao lazy - khong cham KPI khoi dong. Mermaid render trong ca
  Markdown preview va bao cao .greport.md; xuat PNG 1x/2x/3x, SVG,
  copy-as-image; theme brand qua %%{init}%%. FR-KNW-905/915 nay dung
  chung engine voi Studio.
- TRONG TAM CHIEN LUOC v2.3 (Graph/RAG/GraphRAG + Chat luong du lieu):
  (a) Data Quality Report (FR-DQR-001...006, Phase 2-3): rules
  .gquality.yaml 10 loai (cot / lien cot expr SQL / lien file
  foreign_key), diem 6 chieu cong thuc mo (completeness/validity/
  uniqueness/consistency/accuracy-proxy MAD/timeliness), block
  ```quality trong .greport.md, TROI DAT (.gquality.history.jsonl +
  canh bao nguong), QUALITY GATE CLI exit 0/1/2 + JSON cho CI, vong
  khep kin detect -> clean -> verify voi Ban lam sach.
  (b) GraphRAG sau: chunk quality (922 - shingling Jaccard, khong
  embedding), entity resolution mot-changeset (923), graph quality
  report (924), HYBRID RETRIEVAL score = a*BM25 + (1-a)*tin hieu do
  thi k-hop (925, bat bien a=1 = BM25), Golden Set Builder <= 3
  click/record (926).
  (c) Mining sau: group-by mining nguong rieng tung nhom (MIN-007),
  phan ra dong gop da bien leave-one-out (MIN-008).
  (d) TAI UU TIEN: 8 FR loi Graph/RAG len P1, keo Phase som
  (JSONL/Chunk Inspector/BM25 Lab/Graph Preview/Cypher -> Phase 3).
